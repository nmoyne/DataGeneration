...
%
%   Use PFIELD3 for planar 2-D (matrix) arrays.
%
...
%   Y=0 (2-D acoustics in the X-Z plane). The computation is faster in 2-D.
%
...
%   [4 64]).
%   Note: Use TXDELAY to create standard transmit delays (focused, plane,
%         or diverging waves).
%
...
%   4)  PARAM.fe: excitation frequency (in Hz)
%            The default is PARAM.fc
%   5)  PARAM.focus: elevation focus (in m, ignored if Y is not given)
%            The default is Inf (no elevation focusing)
%   6)  PARAM.height: element height (in m, ignored if Y is not given)
%            The default is Inf (no elevation focusing)
%   7)  PARAM.radius: radius of curvature of the probe (in m)
%            The default is Inf (rectilinear array)
%   8)  PARAM.bandwidth: pulse-echo 6dB fractional bandwidth (in %)
%            The default is 75%.
%   9)  PARAM.baffle: property of the baffle:
%            'soft' (default), 'rigid', or a scalar > 0.
%            See "Note on BAFFLE properties" below for details
%
%       *** MEDIUM PARAMETERS ***
%   10) PARAM.c: longitudinal velocity (in m/s, default = 1540 m/s)
%   11) PARAM.attenuation: attenuation coefficient (dB/cm/MHz, default: 0)
%            Notes: A linear frequency-dependence is assumed.
%                   A typical value for soft tissues is ~0.5 dB/cm/MHz.
%
%       *** TRANSMIT PARAMETERS ***
%   12) PARAM.TXapodization: transmit apodization (default: no apodization)
%   13) PARAM.TXnow: number of wavelengths of the TX pulse (default: 1)
%   14) PARAM.TXfreqsweep: frequency sweep for a linear chirp (default: [])
...
%
%   See also TXDELAY, SIMUS, GETPARAM, PFIELD3, GETPULSE, CITE.
%
%   -- Damien Garcia -- 2015/02, last update 2024/10/16
%   website: <a
...
%-- 8) Excitation frequency (in Hz)
if ~isfield(param,'fe')
    param.fe = param.fc;
end
fe = param.fe; % excitation frequency (in Hz)
assert(abs(fe-fc)<(fc*param.bandwidth/200),...
    'The excitation frequency is outside the transducer''s bandwidth')
...
%-- 10) Longitudinal velocity (in m/s)
if ~isfield(param,'c')
    param.c = 1540; % default value
end
c = param.c; % speed of sound (m/s)

%-- 11) Attenuation coefficient (in dB/cm/MHz)
...
%-- 12) Transmit apodization (no unit)
...
%-- 13) TX pulse: Number of wavelengths
...
%-- 14) TX pulse: Frequency sweep for a linear chirp
...
if isempty(FreqSweep)
    ...
    T = NoW/fe; % temporal pulse width
    if isinf(T); T = 1e6; end
    we = 2*pi*fe;
    pulseSpectrum = @(w) 1i*(mysinc(T*(w-we)/2)-mysinc(T*(w+we)/2));
else
    ...
    we = 2*pi*fe;
    dw = 2*pi*FreqSweep;
    s2 = @(w) sqrt(pi*T/dw)*exp(-1i*(w-we).^2*T/2/dw).*...
        (fresnelint((dw/2+w-we)/sqrt(pi*dw/T)) + fresnelint((dw/2-w+we)/sqrt(pi*dw/T)));
...
%-- FREQUENCY SAMPLES
Nf = 2*ceil(fc/df)+1; % number of frequency samples
f = linspace(0,2*fc,Nf); % frequency samples
...
for k = 1:nSampling
    ...
    if ElevationFocusing && ismember(k,k4mgbm)
        % Note with the condition ismember(k,k4mgbm):
    ...
    %{
    if ElevationFocusing
        P = kw/2*(rm-Rf)./(rm*Rf+eps); % instead of: P = kw/2*(1/Rf-1./rm);
        Q = kw*y./rm;
        R = kw*y.^2/2./rm;
        MGBM = (-1)^.25*sqrt(pi)*exp(1i*(4*P.*R+Q.^2)/4./P)/2./sqrt(P).*...
            (myerfi((-1)^(3/4)*(Q-ElementHeight*P)/2./sqrt(P),64) -...
            myerfi((-1)^(3/4)*(Q+ElementHeight*P)/2./sqrt(P),64));
    end
    %}
...
switch n
    ...
    case 25
        ...
        % Journal of the Acoustical Society of America. 2008
        % May;123(5):3516.        
        A = [0.0250 + 0.0903i ; -0.1722 + 0.0417i ; -0.2335 - 0.2073i ;...
            0.0169 - 0.4669i ; 0.5003 - 0.4462i ; 0.9303 + 0.0207i ;...
            0.9359 + 0.8547i ; 0.2283 + 1.7047i ; -1.2466 + 2.0209i ;...
            -3.1897 + 1.1364i ; -5.3844 - 1.7978i ; -3.2923 - 12.5813i ;...
            22.7593];
        A = [A; conj(A(end-1:-1:1))];
        B = [2.7353 + 64.7628i ; 2.4159 + 60.8489i ; 3.0518 + 55.6123i ;...
            3.3108 + 50.1677i ; 3.5307 + 44.6995i ; 3.7217 + 39.1628i ;...
            3.8727 + 33.5837i ; 3.9930 + 27.9784i ; 4.0952 + 22.3454i ;...
            4.1578 + 16.6841i ; 4.2756 + 10.9041i ; 4.6409 + 5.2178i ;...
            4.9426];
        B = 4*[B; conj(B(end-1:-1:1))];
end
...
switch inputname(1)
    case 'param'
        fieldLIST = {'attenuation','baffle','bandwidth','c','fc','fe',...
